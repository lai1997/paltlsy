...
__device__ void makeAprUnqChkCSs(
    UINT_64 *CS,
    UINT_64 &hCS,
    UINT_64 &lCS,
    const int numOfTraces,
    const int AprUnqChkTyp,
    const int lenSum)
{

    if (lenSum > 126) {

        // we need an approximate uniqueness check
    ...
    UINT_64 *d_LTLcache)
{
    ...
    UINT_64 hCS{}, lCS{};
    makeAprUnqChkCSs(CS, hCS, lCS, numOfTraces, AprUnqChkTyp, lenSum);
    ...
    int *d_FinalLTLIdx)
{
    ...
    if (tid < (idx4 - idx3 + 1) * (idx2 - idx1 + 1)) {
        ...
        if (onTheFly) {
            ...
            UINT_64 hCS1{}, lCS1{};
            makeAprUnqChkCSs(CS1, hCS1, lCS1, numOfTraces, AprUnqChkTyp, lenSum);

            UINT_64 hCS2{}, lCS2{};
            makeAprUnqChkCSs(CS2, hCS2, lCS2, numOfTraces, AprUnqChkTyp, lenSum);
...
struct score {
    ...
    UINT_64 hPosBits, lPosBits, hNegBits, lNegBits;

    score(const UINT_64 lP, const UINT_64 hP, const UINT_64 lN, const UINT_64 hN) {
        hPosBits = hP;
        lPosBits = lP;
        hNegBits = hN;
        lNegBits = lN;
    ...
    __host__ __device__ thrust::tuple<int, size_t> operator()(thrust::tuple<thrust::pair<UINT_64, UINT_64>, size_t> t) {

        const UINT_64 hREc = thrust::get<0>(thrust::get<0>(t));
        const UINT_64 lREc = thrust::get<1>(thrust::get<0>(t));

        int bits = 0;

        if ((~hREc & hNegBits) == hNegBits && (~lREc & lNegBits) == lNegBits) {

            UINT_64 hPosPartial = hREc & hPosBits;
            UINT_64 lPosPartial = lREc & lPosBits;

            while (hPosPartial > 0) {
                bits++;
                hPosPartial = hPosPartial & (hPosPartial - 1);
            }
            while (lPosPartial > 0) {
                bits++;
                lPosPartial = lPosPartial & (lPosPartial - 1);
            }

        }

        return thrust::make_tuple(bits, thrust::get<1>(t));
    }
};

struct lowestCostHighestScore {
...
std::string LTLI(const unsigned short *costFun, const unsigned short maxCost, const unsigned int AprUnqChkTyp,
                 const unsigned int NegType, const std::set<char> alphabet, int &LTLcost, std::uint64_t &allLTLs,
                 const std::vector<std::vector<std::string>> pos, const std::vector<std::vector<std::string>> neg) {
    ...
    hashSetsInitialisation<hash_set_t><<<1, alphabetSize>>>(numOfTraces, AprUnqChkTyp, lenSum, cHashSet, iHashSet, d_LTLcache);
        ...
        if (LTLcost - c2 >= c1) {
            ...
            if (N) {
                ...
                do {
                    ...
                    #endif
                    int Blc = (N + 1023) / 1024;
                    Negation<hash_set_t><<<Blc, 1024>>>(x, y, numOfP, numOfN, AprUnqChkTyp, lenSum, onTheFly, d_LTLcache, d_temp_LTLcache,
        ...
        for (int i = c1; 2 * i <= LTLcost - c3; ++i) {
            ...
            if (N) {
                ...
                do {
                    ...
                    #endif
                    int Blc = (N + 1023) / 1024;
                    Intersection<hash_set_t><<<Blc, 1024>>>(Idx1, Idx2, x, y, numOfP, numOfN, AprUnqChkTyp, lenSum, onTheFly, d_LTLcache, d_temp_LTLcache,
                                                            d_temp_leftIdx, d_temp_rightIdx, cHashSet, iHashSet, d_FinalLTLIdx);
                    checkCuda( cudaPeekAtLastError() );
                    checkCuda( cudaMemcpy(FinalLTLIdx, d_FinalLTLIdx, sizeof(int), cudaMemcpyDeviceToHost) );
                    allLTLs += N;
        ...
        for (int i = c1; 2 * i <= LTLcost - c4; ++i) {
            ...
            if (N) {
                ...
                do {
                    ...
                    #endif
                    int Blc = (N + 1023) / 1024;
                    Union<hash_set_t><<<Blc, 1024>>>(Idx1, Idx2, x, y, numOfP, numOfN, AprUnqChkTyp, lenSum, onTheFly, d_LTLcache, d_temp_LTLcache,
                                                     d_temp_leftIdx, d_temp_rightIdx, cHashSet, iHashSet, d_FinalLTLIdx);
                    checkCuda( cudaPeekAtLastError() );
                    checkCuda( cudaMemcpy(FinalLTLIdx, d_FinalLTLIdx, sizeof(int), cudaMemcpyDeviceToHost) );
                    allLTLs += N;
        ...
        if (LTLcost - c5 >= c1) {
            ...
            if (N) {
                ...
                do {
                    ...
                    #endif
                    int Blc = (N + 1023) / 1024;
                    Next<hash_set_t><<<Blc, 1024>>>(x, y, numOfP, numOfN, AprUnqChkTyp, lenSum, onTheFly, d_LTLcache, d_temp_LTLcache,
                                                    d_temp_leftIdx, d_temp_rightIdx, cHashSet, iHashSet, d_FinalLTLIdx);
                    checkCuda( cudaPeekAtLastError() );
                    checkCuda( cudaMemcpy(FinalLTLIdx, d_FinalLTLIdx, sizeof(int), cudaMemcpyDeviceToHost) );
                    allLTLs += N;
        ...
        if (LTLcost - c6 >= c1) {
            ...
            if (N) {
                ...
                do {
                    ...
                    #endif
                    int Blc = (N + 1023) / 1024;
                    Finally<hash_set_t><<<Blc, 1024>>>(x, y, numOfP, numOfN, AprUnqChkTyp, lenSum, onTheFly, d_LTLcache, d_temp_LTLcache,
                                                       d_temp_leftIdx, d_temp_rightIdx, cHashSet, iHashSet, d_FinalLTLIdx);
                    checkCuda( cudaPeekAtLastError() );
                    checkCuda( cudaMemcpy(FinalLTLIdx, d_FinalLTLIdx, sizeof(int), cudaMemcpyDeviceToHost) );
                    allLTLs += N;
        ...
        if (LTLcost - c7 >= c1) {
            ...
            if (N) {
                ...
                do {
                    ...
                    #endif
                    int Blc = (N + 1023) / 1024;
                    Globally<hash_set_t><<<Blc, 1024>>>(x, y, numOfP, numOfN, AprUnqChkTyp, lenSum, onTheFly, d_LTLcache, d_temp_LTLcache,
                                                        d_temp_leftIdx, d_temp_rightIdx, cHashSet, iHashSet, d_FinalLTLIdx);
                    checkCuda( cudaPeekAtLastError() );
                    checkCuda( cudaMemcpy(FinalLTLIdx, d_FinalLTLIdx, sizeof(int), cudaMemcpyDeviceToHost) );
                    allLTLs += N;
        ...
        for (int i = c1; 2 * i <= LTLcost - c8; ++i) {
            ...
            if (N) {
                ...
                do {
                    ...
                    #endif
                    int Blc = (N + 1023) / 1024;
                    Until<hash_set_t><<<Blc, 1024>>>(Idx1, Idx2, x, y, numOfP, numOfN, AprUnqChkTyp, lenSum, onTheFly, d_LTLcache, d_temp_LTLcache,
                                                     d_temp_leftIdx, d_temp_rightIdx, cHashSet, iHashSet, d_FinalLTLIdx);
                    checkCuda( cudaPeekAtLastError() );
                    checkCuda( cudaMemcpy(FinalLTLIdx, d_FinalLTLIdx, sizeof(int), cudaMemcpyDeviceToHost) );
                    allLTLs += 2 * N;
...
int main (int argc, char *argv[]) {
    ...
    if (argc != 13) {
        ...
        printf("-----------------------------------------------------------------\n");
        printf("%s <input_file_address> <c1> <c2> <c3> <c4> <c5> <c6> <c7> <c8> <maxCost> <AprUnqChkTyp> <NegType>\n", argv[0]);
        printf("-----------------------------------------------------------------\n");
        printf("\nFor example\n");
        printf("-----------------------------------------------------------------\n");
        printf("%s ./input.txt 1 1 1 1 1 1 1 1 500\n 1 1", argv[0]);
        printf("-----------------------------------------------------------------\n");
        return 0;
    }
    ...
    if (std::atoi(argv[11]) < 1 || std::atoi(argv[11]) > 3) {
        printf("The argument number 10, AprUnqChkTyp = \"%s\", should be 1, 2, or 3.\n", argv[11]);
        argError = true;
    }
    if (std::atoi(argv[12]) < 1 || std::atoi(argv[12]) > 2) {
        printf("The last argument, NegType = \"%s\", should be 1, or 2.\n", argv[12]);
        argError = true;
    }
    ...
    unsigned int AprUnqChkTyp = std::atoi(argv[11]);
    ...
    std::string output = LTLI(costFun, maxCost, AprUnqChkTyp, NegType, alphabet, LTLcost, allLTLs, pos, neg);
